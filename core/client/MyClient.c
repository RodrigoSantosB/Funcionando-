...
int main() 
{
    ...
    if (!windowInit(LARGURA, ALTURA, "CINHALLA!"))
        return -1;
            ...
            if (lobbyEvent.type == ALLEGRO_EVENT_KEY_DOWN)
            {
                switch(lobbyEvent.keyboard.keycode)
                {
                    case ALLEGRO_KEY_ENTER:
                        ...
                        sendMsgToServer(&pacote, sizeof(ServerPack));
                        lobbyMessage[0]='\0';
                        break;
                }
            }
...
void printConnectScreen(char str[])
{
    ...
    al_draw_text(start, al_map_rgb(165,42,42), WIDTH / 2.4, 120, ALLEGRO_ALIGN_CENTRE, "Digite o IP do server:");

    if (strlen(str) > 0)
    {
        al_draw_text(start, al_map_rgb(165,42,42), WIDTH / 2.4,
                     (HEIGHT - al_get_font_ascent(start)) / 1.8,
...
void printLobbyText(char str[])
{
    ...
    al_draw_text(start, al_map_rgb(165,42,42), 2.4, 120, ALLEGRO_ALIGN_LEFT, "vo trancar o curso");

    if (strlen(str) > 0)
    {
        al_draw_text(start, al_map_rgb(165,42,42), 5,
                     (HEIGHT - al_get_font_ascent(start)) - 90,
...
void printLoginScreen(char str[])
{
    ...
    al_draw_text(start, al_map_rgb(165,42,42), WIDTH / 2.4, 120, ALLEGRO_ALIGN_CENTRE, "Digite o Login desejado: ");
